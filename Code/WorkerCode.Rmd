...
```{r}
setwd("/Users/kellyhuang/Desktop")
getwd()
#install.packages("haven")
library(haven)
Rawdata <- haven::read_sav("Data-NFEHRS_revised.sav")
```
...
lm1<-lm(formula = pvlitM ~ ED_Level + act_lrn + age_cat + work_flexM + 
    pub_priv + GENDER_R + NFEHRS, data = data_lit)

lm2<-lm(formula = pvlitM ~ ED_Level + act_lrn + age_cat + work_flexM + GENDER_R + NFEHRS, data = data_lit)

anova(lm1,lm2) # p-value=0.01978, we confirm that pub/private sector is significant.
...
anova(lm.num.without,lm.num.with) # p-value = 0.0037**
```
...
anova(lm.lit.without,lm.lit.with) # p-value=0.0185, we confirm that pub/private sector is significant.
```