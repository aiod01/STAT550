...
{
  tb<-table(newdata$GENDER_R,newdata$AGE_R)
}
'$'(newdata , "GENDER_R")
tb<-chisq.test($`(newdata , "GENDER_R"),newdata[,"AGE_R"])
...
```
```{r}
death_table = matrix(c("dependent", "dependent", "dependent", "dependent"), ncol=2)
colnames(death_table) = c('ed_level', 'age')
rownames(death_table) = c('ed_level', 'age')
