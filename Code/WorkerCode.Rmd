...
(step(null.num,scope = list(lower=null.num,upper=full.num), direction="forward", criterion 
     = "AIC", trace = T))$coefficients #ED_Level, act_lrn, Age, Gender, public/private, work_flexM, NFEHRS
...
anova(lm(formula = pvnumM ~ ED_Level + act_lrn + AGE_R+ work_flexM + GENDER_R +pub_priv, data = data_num),lm(formula = pvnumM ~ ED_Level + act_lrn + AGE_R + work_flexM + GENDER_R +  pub_priv +FNFAET12JR + NFE12 + FNFAET12NJR , data = data_num))$`Pr(>F)`[2]  # 0.15948
