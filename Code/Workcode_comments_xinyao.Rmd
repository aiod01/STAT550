...
```
**$\emph{Comment1:}$ For data input, you can juse "read_sav" function directly. For example, my data is on my desktop, and I can input the data by the above code easily.**
...
**$\emph{Comment3}:$ For this part, you find some data out of our range of response data. I think it's better you count how many obeservations whose num_use=7. How do you find that only one observation has num_use=7? I will recommend the following code. Since you can find the position of the artefact data and can delete them from your rawdata directly.**
```{r}
...
**Comment12: For me, I usually construct a full mode and then use the step function directly. Becase you can set the parameters in this step function. For example, step(full.num,method="backward") or step(full.num,method="forward"). That's more concise.**
...
**Comment15: Add some comments here. It's really hard for the readers to know what's going on in this part. Based on my understanding, you get two models A and B using stepwise AIC(forward and backward) and A is nested in B. Why not to use ANOVA to compare A and B first? As for the part ANOVA: With vs without one variable, I have some doubts about that. I give details in my comments of report. My suggestions are as follows: First, use anova to compare model A and B to find whether they are significantly different. Then deleting NFEHRS from A get the model C. Use anova to compare A and C. Based on your results to draw some conclusions. **
